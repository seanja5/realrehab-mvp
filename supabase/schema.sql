...
-- --------------------------------------------------------------------
-- Patient schedule slots (personalized weekly schedule per patient)
-- Each slot = 30-minute block (day_of_week + slot_time = start time)
...
create policy patient_schedule_slots_select_own
  on accounts.patient_schedule_slots for select
  using (
    accounts.is_admin()
...
create policy patient_schedule_slots_insert_own
  on accounts.patient_schedule_slots for insert
  with check (
    accounts.is_admin()
    or (accounts.is_patient() and patient_profile_id = accounts.current_patient_profile_id())
  );
...
create policy patient_schedule_slots_update_own
  on accounts.patient_schedule_slots for update
  using (
    accounts.is_admin()
    or (accounts.is_patient() and patient_profile_id = accounts.current_patient_profile_id())
  )
...
create policy patient_schedule_slots_delete_own
  on accounts.patient_schedule_slots for delete
  using (
    accounts.is_admin()
    or (accounts.is_patient() and patient_profile_id = accounts.current_patient_profile_id())
  );
